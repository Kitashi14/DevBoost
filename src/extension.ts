...
interface Button {
	name: string;
	cmd: string;
	description?: string;           // Detailed description of what the button does
...
class ButtonsTreeProvider implements vscode.TreeDataProvider<TreeItemBase> {
	...
	async loadButtons(): Promise<void> {
		...
		// Load global buttons
		const globalButtons = this.context.globalState.get<Button[]>('devboost.globalButtons') || [];
		this.buttons.push(...globalButtons.map(b => ({ ...b, scope: 'global' as const })));
	...
	async addButtons(buttons: Button[], scope: 'workspace' | 'global'): Promise<number> {
		...
		const duplicateButtons: Array<{newName: string, existingName: string}> = [];
	...
	private async checkDuplicate(newButton: Button, targetScope: 'workspace' | 'global'): Promise<string | null> {
		...
		try {
			...
			const existingButtonsInfo = buttonsToCheck.map((b, i) => 
				`${i + 1}. Name: "${b.name}", Command: "${b.cmd}", Description: "${b.description || 'N/A'}", Scope: ${b.scope}`
			).join('\n');
	...
	async deleteButton(item: ButtonTreeItem): Promise<void> {
		...
		try {
			if (button.scope === 'global') {
				await this.context.globalState.update('devboost.globalButtons', 
					this.buttons.filter(b => b.scope === 'global').map(({ scope, ...b }) => b));
			} else {
	...
	private async saveButtons(buttons: Button[], scope: 'workspace' | 'global'): Promise<void> {
		if (scope === 'global') {
			const existingButtons = this.context.globalState.get<Button[]>('devboost.globalButtons') || [];
...
function getSystemInfo(): { platform: string; shell: string} {
	...
	// Get AI suggestions from GitHub Copilot
async function getAISuggestions(topActivities: string[]): Promise<Button[]> {
	try {
...
async function getCustomButtonSuggestion(description: string): Promise<Button | null> {
	try {
...
function getFallbackSuggestions(topActivities: string[]): Button[] {
